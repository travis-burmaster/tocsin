...
**Status: implemented, not released.** The CLI, all four scan scopes (`--brew`, `--files`, `--project`, `--posture`), and `doctor` work end to end on macOS and are covered by an automated test suite. This repository is still private; the project is intended for a future open-source release, and no PyPI package name is currently reserved for it. Before that release: the ClamAV file-scanning adapter has never been exercised against a real clamscan engine (no engine was available during development -- see `docs/evidence/validation.md`), and Intel Mac coverage is untested. MIT is the proposed code license, subject to dependency and distribution review before release; there is no `LICENSE` file yet.
